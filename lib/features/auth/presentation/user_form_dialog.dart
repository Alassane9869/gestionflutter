...
class _UserFormDialogState extends ConsumerState<UserFormDialog> {
  ...
  void _updatePermission({
    ...
    setState(() {
      _permissions = _permissions.copyWith(
        ...
        canUseAi: canUseAi ?? _permissions.canUseAi,
        canViewGlobalSalesHistory: canViewGlobalSalesHistory ?? _permissions.canViewGlobalSalesHistory,
        canViewDashboard: canViewDashboard ?? _permissions.canViewDashboard,
        canManageProducts: canManageProducts ?? _permissions.canManageProducts,
        canDeleteSales: canDeleteSales ?? _permissions.canDeleteSales,
      );
    });
  }
  ...
  List<_PermissionItem> _getPermissionList() {
    // NOTE : canManageUsers est volontairement omis de cette liste pour des raisons de sécurité.
    // Seul un Administrateur principal (rôle Admin) possède ce droit de manière innée,
    // et il ne doit pas pouvoir être délégué à d'autres rôles intermédiaires via l'UI.
    return [
      ...
      _PermissionItem("Mouvements de Stock", _permissions.canManageStock, (v) => _updatePermission(canManageStock: v)),
      ...
      _PermissionItem("Utiliser l'Assistant IA", _permissions.canUseAi, (v) => _updatePermission(canUseAi: v)),
      _PermissionItem("Voir Historique Global Ventes", _permissions.canViewGlobalSalesHistory, (v) => _updatePermission(canViewGlobalSalesHistory: v)),
      _PermissionItem("Accéder au Tableau de bord", _permissions.canViewDashboard, (v) => _updatePermission(canViewDashboard: v)),
      _PermissionItem("Créer/Modifier les Articles", _permissions.canManageProducts, (v) => _updatePermission(canManageProducts: v)),
      _PermissionItem("Supprimer Définitiv. Ventes", _permissions.canDeleteSales, (v) => _updatePermission(canDeleteSales: v)),
    ];
  }
