...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  Map<String, bool> _copilotPermissions = {};
  ...
  final _elevenLabsVoiceIdCtrl = TextEditingController();
  final _labsLicenseKeyCtrl = TextEditingController();
  ...
  final _cloudSyncKeyCtrl = TextEditingController();
  final _cloudEndpointCtrl = TextEditingController();
  ...
  final List<IconData> _menuIcons = [
    ...
    FluentIcons.chat_24_regular,               // 15: WhatsApp Cloud API
  ];
  ...
  Future<void> _loadLicense() async {
    ...
    final labsKey = prefs.getString('labs_activation_key') ?? '';
    if (mounted) {
      setState(() {
        _daysRemaining = days;
        _labsLicenseKeyCtrl.text = labsKey;
      });
    }
    _loadDbStats();
    _loadAutoBackups();
  ...
  void _syncFromProvider(ShopSettings s) {
    ...
    if (_quotePrinter != s.quotePrinterName) _quotePrinter = s.quotePrinterName;
    if (_purchaseOrderPrinter != s.purchaseOrderPrinterName) _purchaseOrderPrinter = s.purchaseOrderPrinterName;
    if (_labelPrinter != s.labelPrinterName) _labelPrinter = s.labelPrinterName;
    if (_contractPrinter != s.contractPrinterName) _contractPrinter = s.contractPrinterName;
    if (_payrollPrinter != s.payrollPrinterName) _payrollPrinter = s.payrollPrinterName;
    if (_reportPrinter != s.reportPrinterName) _reportPrinter = s.reportPrinterName;
    if (_proformaPrinter != s.proformaPrinterName) _proformaPrinter = s.proformaPrinterName;
    if (_deliveryPrinter != s.deliveryPrinterName) _deliveryPrinter = s.deliveryPrinterName;

    if (_currency != s.currency) _currency = s.currency;
    if (_removeDecimals != s.removeDecimals) _removeDecimals = s.removeDecimals;
    if (_showQrCode != s.showQrCode) _showQrCode = s.showQrCode;
    if (_useTax != s.useTax) _useTax = s.useTax;
    if (_autoBackupEnabled != s.autoBackupEnabled) _autoBackupEnabled = s.autoBackupEnabled;
    if (_useAutoRef != s.useAutoRef) _useAutoRef = s.useAutoRef;
    if (_loyaltyEnabled != s.loyaltyEnabled) _loyaltyEnabled = s.loyaltyEnabled;
    if (_autoPrintTicket != s.autoPrintTicket) _autoPrintTicket = s.autoPrintTicket;
    if (_showPreviewBeforePrint != s.showPreviewBeforePrint) _showPreviewBeforePrint = s.showPreviewBeforePrint;
    if (_openCashDrawer != s.openCashDrawer) _openCashDrawer = s.openCashDrawer;
    if (_directPhysicalPrinting != s.directPhysicalPrinting) _directPhysicalPrinting = s.directPhysicalPrinting;

    // Layout templates
    if (_receipt != s.defaultReceipt) _receipt = s.defaultReceipt;
    if (_invoice != s.defaultInvoice) _invoice = s.defaultInvoice;
    if (_quote != s.defaultQuote) _quote = s.defaultQuote;
    if (_purchaseOrder != s.defaultPurchaseOrder) _purchaseOrder = s.defaultPurchaseOrder;
    if (_logoPath != s.logoPath) _logoPath = s.logoPath;

    // Dropdown formats & model states
    if (_refModel != s.refModel) _refModel = s.refModel;
    if (_barcodeModel != s.barcodeModel) _barcodeModel = s.barcodeModel;
    if (_labelFormat != s.labelFormat) _labelFormat = s.labelFormat;
    if (_roundingMode != s.roundingMode) _roundingMode = s.roundingMode;
    if (_assistantLevel != s.assistantLevel) _assistantLevel = s.assistantLevel;

    // Boolean flags
    if (_showPriceOnLabels != s.showPriceOnLabels) _showPriceOnLabels = s.showPriceOnLabels;
    if (_showNameOnLabels != s.showNameOnLabels) _showNameOnLabels = s.showNameOnLabels;
    if (_showSkuOnLabels != s.showSkuOnLabels) _showSkuOnLabels = s.showSkuOnLabels;
    if (_autoPrintLabelsOnStockIn != s.autoPrintLabelsOnStockIn) _autoPrintLabelsOnStockIn = s.autoPrintLabelsOnStockIn;
    if (_enableSounds != s.enableSounds) _enableSounds = s.enableSounds;
    if (_enableAppSounds != s.enableAppSounds) _enableAppSounds = s.enableAppSounds;
    if (_enableCustomerDisplaySounds != s.enableCustomerDisplaySounds) _enableCustomerDisplaySounds = s.enableCustomerDisplaySounds;
    if (_useCustomerDisplay3D != s.useCustomerDisplay3D) _useCustomerDisplay3D = s.useCustomerDisplay3D;
    if (_isVoiceEnabled != s.isVoiceEnabled) _isVoiceEnabled = s.isVoiceEnabled;
    if (_enableVoiceConfig != s.enableVoiceConfig) _enableVoiceConfig = s.enableVoiceConfig;
    if (_showAssistant != s.showAssistant) _showAssistant = s.showAssistant;
    ...
    if (_sloganCtrl.text != s.slogan) _sloganCtrl.text = s.slogan;
    if (_phoneCtrl.text != s.phone) _phoneCtrl.text = s.phone;
    if (_whatsappCtrl.text != s.whatsapp) _whatsappCtrl.text = s.whatsapp;
    if (_addressCtrl.text != s.address) _addressCtrl.text = s.address;
    if (_emailCtrl.text != s.email) _emailCtrl.text = s.email;
    if (_rcCtrl.text != s.rc) _rcCtrl.text = s.rc;
    if (_nifCtrl.text != s.nif) _nifCtrl.text = s.nif;
    if (_bankAccountCtrl.text != s.bankAccount) _bankAccountCtrl.text = s.bankAccount;
    if (_taxNameCtrl.text != s.taxName) _taxNameCtrl.text = s.taxName;
    if (_taxRateCtrl.text != s.taxRate.toString()) _taxRateCtrl.text = s.taxRate.toString();
    if (_refPrefixCtrl.text != s.refPrefix) _refPrefixCtrl.text = s.refPrefix;
    if (_labelWidthCtrl.text != s.labelWidth.toString()) _labelWidthCtrl.text = s.labelWidth.toString();
    if (_labelHeightCtrl.text != s.labelHeight.toString()) _labelHeightCtrl.text = s.labelHeight.toString();
    if (_paymentMethodsCtrl.text != s.paymentMethods.join(', ')) _paymentMethodsCtrl.text = s.paymentMethods.join(', ');
    if (_receiptFooterCtrl.text != s.receiptFooter) _receiptFooterCtrl.text = s.receiptFooter;
    
    // AI Api Keys
    if (_deepSeekApiKeyCtrl.text != s.deepSeekApiKey) _deepSeekApiKeyCtrl.text = s.deepSeekApiKey;
    if (_geminiApiKeyCtrl.text != s.geminiApiKey) _geminiApiKeyCtrl.text = s.geminiApiKey;
    if (_elevenLabsApiKeyCtrl.text != s.elevenLabsApiKey) _elevenLabsApiKeyCtrl.text = s.elevenLabsApiKey;
    if (_elevenLabsVoiceIdCtrl.text != s.elevenLabsVoiceId) _elevenLabsVoiceIdCtrl.text = s.elevenLabsVoiceId;

    // Lock screen time
    if (_autoLockMinutesCtrl.text != s.autoLockMinutes.toString()) _autoLockMinutesCtrl.text = s.autoLockMinutes.toString();

    // Legal Form & Business Policies
    if (_legalFormCtrl.text != s.legalForm) _legalFormCtrl.text = s.legalForm;
    if (_capitalCtrl.text != s.capital) _capitalCtrl.text = s.capital;
    if (_warrantyCtrl.text != s.policyWarranty) _warrantyCtrl.text = s.policyWarranty;
    if (_returnsCtrl.text != s.policyReturns) _returnsCtrl.text = s.policyReturns;
    if (_paymentsPolicyCtrl.text != s.policyPayments) _paymentsPolicyCtrl.text = s.policyPayments;
    if (_validityCtrl.text != s.quoteValidityDays.toString()) _validityCtrl.text = s.quoteValidityDays.toString();
    if (_legalNoteCtrl.text != s.invoiceLegalNote) _legalNoteCtrl.text = s.invoiceLegalNote;
    if (_managerPinCtrl.text != s.managerPin) _managerPinCtrl.text = s.managerPin;
    if (_maxDiscountThresholdCtrl.text != s.maxDiscountThreshold.toString()) _maxDiscountThresholdCtrl.text = s.maxDiscountThreshold.toString();
    if (_vipThresholdCtrl.text != s.vipThreshold.toString()) _vipThresholdCtrl.text = s.vipThreshold.toString();

    // Loyalty Points Configuration
    if (_pointsPerAmountCtrl.text != s.pointsPerAmount.toString()) _pointsPerAmountCtrl.text = s.pointsPerAmount.toString();
    if (_amountPerPointCtrl.text != s.amountPerPoint.toString()) _amountPerPointCtrl.text = s.amountPerPoint.toString();

    // Margins
    if (_marginTicketTopCtrl.text != s.marginTicketTop.toString()) _marginTicketTopCtrl.text = s.marginTicketTop.toString();
    if (_marginTicketBottomCtrl.text != s.marginTicketBottom.toString()) _marginTicketBottomCtrl.text = s.marginTicketBottom.toString();
    if (_marginTicketLeftCtrl.text != s.marginTicketLeft.toString()) _marginTicketLeftCtrl.text = s.marginTicketLeft.toString();
    if (_marginTicketRightCtrl.text != s.marginTicketRight.toString()) _marginTicketRightCtrl.text = s.marginTicketRight.toString();

    if (_marginInvoiceTopCtrl.text != s.marginInvoiceTop.toString()) _marginInvoiceTopCtrl.text = s.marginInvoiceTop.toString();
    if (_marginInvoiceBottomCtrl.text != s.marginInvoiceBottom.toString()) _marginInvoiceBottomCtrl.text = s.marginInvoiceBottom.toString();
    if (_marginInvoiceLeftCtrl.text != s.marginInvoiceLeft.toString()) _marginInvoiceLeftCtrl.text = s.marginInvoiceLeft.toString();
    if (_marginInvoiceRightCtrl.text != s.marginInvoiceRight.toString()) _marginInvoiceRightCtrl.text = s.marginInvoiceRight.toString();

    if (_marginLabelXCtrl.text != s.marginLabelX.toString()) _marginLabelXCtrl.text = s.marginLabelX.toString();
    if (_marginLabelYCtrl.text != s.marginLabelY.toString()) _marginLabelYCtrl.text = s.marginLabelY.toString();

    // Network / Sync Port and IP
    if (_networkMode != s.networkMode) _networkMode = s.networkMode;
    if (_serverIpCtrl.text != s.serverIp) _serverIpCtrl.text = s.serverIp;
    if (_serverPortCtrl.text != s.serverPort.toString()) _serverPortCtrl.text = s.serverPort.toString();

    // Customer display settings
    if (_customerDisplayTheme != s.customerDisplayTheme) _customerDisplayTheme = s.customerDisplayTheme;
    if (_enableCustomerDisplayTicker != s.enableCustomerDisplayTicker) _enableCustomerDisplayTicker = s.enableCustomerDisplayTicker;
    if (_customerDisplayMessagesCtrl.text != s.customerDisplayMessages.join('\n')) _customerDisplayMessagesCtrl.text = s.customerDisplayMessages.join('\n');

    // Cloud / Email Backups and SMTP Settings
    if (_cloudBackupPath != s.cloudBackupPath) _cloudBackupPath = s.cloudBackupPath;
    if (_emailBackupEnabled != s.emailBackupEnabled) _emailBackupEnabled = s.emailBackupEnabled;
    if (_emailBackupFrequency != s.emailBackupFrequency) _emailBackupFrequency = s.emailBackupFrequency;
    if (_emailBackupHour != s.emailBackupHour) _emailBackupHour = s.emailBackupHour;
    if (_backupEmailRecipientCtrl.text != s.backupEmailRecipient) _backupEmailRecipientCtrl.text = s.backupEmailRecipient;
    if (_smtpHostCtrl.text != s.smtpHost) _smtpHostCtrl.text = s.smtpHost;
    if (_smtpPortCtrl.text != s.smtpPort.toString()) _smtpPortCtrl.text = s.smtpPort.toString();
    if (_smtpUserCtrl.text != s.smtpUser) _smtpUserCtrl.text = s.smtpUser;
    if (_smtpPasswordCtrl.text != s.smtpPassword) _smtpPasswordCtrl.text = s.smtpPassword;
    if (_whatsappTokenCtrl.text != s.whatsappToken) _whatsappTokenCtrl.text = s.whatsappToken;
    if (_whatsappPhoneNumberIdCtrl.text != s.whatsappPhoneNumberId) _whatsappPhoneNumberIdCtrl.text = s.whatsappPhoneNumberId;
  }
  ...
  void _initControllers(ShopSettings s) {
    ...
    _proformaPrinter = s.proformaPrinterName;
    _deliveryPrinter = s.deliveryPrinterName;
    ...
    _isVoiceEnabled = s.isVoiceEnabled;
    _enableVoiceConfig = s.enableVoiceConfig;
    ...
    _copilotPermissions = Map<String, bool>.from(s.copilotPermissions);
    ...
    _cloudSyncKeyCtrl.text = s.cloudSyncKey;
    _cloudEndpointCtrl.text = s.cloudEndpoint;
    ...
    _whatsappTokenCtrl.text = s.whatsappToken;
  ...
  Future<void> _save() async {
    ...
    final updated = current.copyWith(
      ...
      copilotPermissions: _copilotPermissions,
      ...
      isAiEnabled: _showAssistant,
      networkMode: _networkMode,
      serverIp: _serverIpCtrl.text.trim(),
      serverPort: int.tryParse(_serverPortCtrl.text) ?? 8080,
      cloudSyncKey: _cloudSyncKeyCtrl.text.trim(),
      cloudEndpoint: _cloudEndpointCtrl.text.trim(),
      ...
      enableVoiceConfig: _enableVoiceConfig,
      ...
      whatsappToken: _whatsappTokenCtrl.text.trim(),
    ...
    final prefs = await SharedPreferences.getInstance();
    if (_showAssistant) {
      await prefs.setString('labs_activation_key', _labsLicenseKeyCtrl.text.trim());
    } else {
      await prefs.remove('labs_activation_key');
    }
  ...
  void _showDeveloperDataDialog(BuildContext context) {
    showDialog(
      context: context,
      ...
      builder: (context) {
        return AlertDialog(
          ...
          title: const Row(
            children: [
              Icon(FluentIcons.brain_circuit_24_filled, color: Colors.blue, size: 28),
              SizedBox(width: 12),
              Text("Configuration Danaya AI", style: TextStyle(fontWeight: FontWeight.w900)),
            ],
          ),
          content: SizedBox(
            width: 480,
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  const Text(
                    "Pour activer les capacités d'intelligence artificielle, veuillez saisir votre licence et vos clés d'activation. Les clés sont stockées localement de manière sécurisée.",
                    style: TextStyle(fontSize: 13, height: 1.4, color: Colors.grey),
                  ),
                  const SizedBox(height: 20),
                  EnterpriseWidgets.buildPremiumTextField(
                    context,
                    ctrl: _labsLicenseKeyCtrl,
                    label: "LICENCE D'ACTIVATION DANAYA AI / LABS",
                    hint: "Saisir la clé de licence Labs...",
                    icon: FluentIcons.shield_keyhole_24_regular,
                  ),
                  const SizedBox(height: 16),
                  EnterpriseWidgets.buildPremiumTextField(
                    context,
                    ctrl: _geminiApiKeyCtrl,
                    label: "CLÉ DE MOTEUR DANAYA AI VIP (RECOMMANDÉ)",
                    hint: "Saisir la clé VIP...",
                    icon: FluentIcons.key_24_regular,
                  ),
                  const SizedBox(height: 16),
                  EnterpriseWidgets.buildPremiumTextField(
                    context,
                    ctrl: _deepSeekApiKeyCtrl,
                    label: "CLÉ DE MOTEUR DANAYA AI STANDARD",
                    hint: "Saisir la clé Standard...",
                    icon: FluentIcons.key_24_regular,
                  ),
                  const SizedBox(height: 16),
                  EnterpriseWidgets.buildPremiumTextField(
                    context,
                    ctrl: _elevenLabsApiKeyCtrl,
                    label: "CLÉ DE MOTEUR VOCAL DANAYA AI",
                    hint: "Saisir la clé de synthèse vocale...",
                  ...
                  EnterpriseWidgets.buildPremiumTextField(
                    ...
                    label: "IDENTIFIANT DE VOIX DANAYA AI",
                    hint: "Saisir l'identifiant de la voix...",
                    icon: FluentIcons.mic_sparkle_24_regular,
                  ),
                ],
              ),
            ),
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                ...
                final validation = service.validateLabsKey(_labsLicenseKeyCtrl.text.trim(), hid);
                if (validation.isValid) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text("Licence Danaya Labs invalide pour cet appareil."),
  ...
  Widget build(BuildContext context) {
    ref.listen<AsyncValue<ShopSettings>>(shopSettingsProvider, (prev, next) {
      if (next.hasValue) {
        ...
        setState(() {
          _syncFromProvider(next.value!);
        });
      }
    });

    _selectedIndex = ref.watch(settingsTabIndexProvider);
    ...
    final contentBg = Theme.of(context).colorScheme.surface;
  ...
  String _getSectionSubtitle(int index) {
    switch (index) {
      ...
      case 15: return "Identifiants Meta Cloud API pour l'envoi WhatsApp.";
      default: return "";
    }
  }
    ...
    switch (_selectedIndex) {
      ...
      case 6: // Afficheur Client (Pro)
        return CustomerDisplaySettingsSection(
          ...
          isVoiceEnabled: _isVoiceEnabled,
          onIsVoiceEnabledChanged: (v) { setState(() => _isVoiceEnabled = v); _saveDebounced(); },
          enableVoiceConfig: _enableVoiceConfig,
          onEnableVoiceConfigChanged: (v) { setState(() => _enableVoiceConfig = v); _saveDebounced(); },
      ...
      case 8: // Automatisation & IA
        return AutomationSettingsSection(
          ...
          showAssistant: _showAssistant,
          onShowAssistantChanged: (v) {
            if (v) {
              _showDeveloperDataDialog(context);
            } else {
              setState(() {
          ...
          copilotPermissions: _copilotPermissions,
          onCopilotPermissionChanged: (key, val) => setState(() { _copilotPermissions[key] = val; _saveDebounced(); }),
          useCloudAi: _useCloudAi,
          onUseCloudAiChanged: (v) => setState(() { _useCloudAi = v; _saveDebounced(); }),
          cloudAiProvider: _cloudAiProvider,
          ...
          elevenLabsVoiceIdCtrl: _elevenLabsVoiceIdCtrl,
          onSaveDebounced: _saveDebounced,
        );
      ...
      case 11: // Système & Réseau
        return SystemMaintenanceSection(
          ...
          networkMode: _networkMode,
          onModeChanged: (NetworkMode? v) { 
            if (v != null) { 
              setState(() => _networkMode = v); 
              if (v != NetworkMode.client) _saveDebounced(); 
            } 
          },
          serverIpCtrl: _serverIpCtrl,
          serverPortCtrl: _serverPortCtrl,
          syncKeyCtrl: _syncKeyCtrl,
          cloudSyncKeyCtrl: _cloudSyncKeyCtrl,
          cloudEndpointCtrl: _cloudEndpointCtrl,
          onSaveDebounced: _saveDebounced,
          isAutoLockEnabled: _isAutoLockEnabled,
      ...
      // case 15: WhatsApp Cloud API — masqué (utilisation du mode local)
  ...
  Future<void> _removeLogo() async {
    final ok = await EnterpriseWidgets.showPremiumConfirmDialog(
      ...
      message: "Le logo sera retiré visuellement.",
      isDestructive: true,
      onConfirm: () {},
    );
    if (ok == true && mounted) {
