...
class SupplierListNotifier extends AsyncNotifier<List<Supplier>> {
  ...
  Future<void> deleteSupplier(String id) async {
    ...
    final settings = ref.read(shopSettingsProvider).value;
    if (settings?.networkMode == NetworkMode.client) {
      ref.read(clientSyncProvider).sendDeleteToServer('suppliers', id);
    }
...
class SuppliersSearchQueryNotifier extends Notifier<String> {
  @override
  String build() => '';
  void update(String val) => state = val;
}
final suppliersSearchQueryProvider = NotifierProvider<SuppliersSearchQueryNotifier, String>(SuppliersSearchQueryNotifier.new);

class SuppliersFilterTabNotifier extends Notifier<String> {
  @override
  String build() => 'ALL';
  void update(String val) => state = val;
}
final suppliersFilterTabProvider = NotifierProvider<SuppliersFilterTabNotifier, String>(SuppliersFilterTabNotifier.new);

class SuppliersSortByNotifier extends Notifier<String> {
  @override
  String build() => 'NAME';
  void update(String val) => state = val;
}
final suppliersSortByProvider = NotifierProvider<SuppliersSortByNotifier, String>(SuppliersSortByNotifier.new);
