    ...
    try {
      ...
      await db.transaction((txn) async {
        ...
        for (var item in cart) {
          ...
          final productRows = await txn.query('products', columns: ['quantity', 'weighted_average_cost', 'purchasePrice', 'is_service'], where: 'id = ?', whereArgs: [productId]);
          bool productIsService = false;
          if (productRows.isNotEmpty) {
            final pRow = productRows.first;
            productIsService = (pRow['is_service'] as int? ?? 0) == 1;

            if (productIsService) {
              // Service: keep quantity at 0.0, only update cost prices
              batch.rawUpdate(
                'UPDATE products SET purchasePrice = ?, weighted_average_cost = ? WHERE id = ?',
          ...
          if (!productIsService) {
            // Record stock movement only for physical products
