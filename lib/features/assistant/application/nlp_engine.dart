...
class NlpEngine {
  ...
  static final Map<String, NlpIntent> _commonPhrases = {
    ...
    'fait un devis': NlpIntent.actionNewQuote,
    'faire un devis': NlpIntent.actionNewQuote,
    'creer un devis': NlpIntent.actionNewQuote,
    'generer un devis': NlpIntent.actionNewQuote,
    'nouveau devis': NlpIntent.actionNewQuote,
    'liste des devis': NlpIntent.quoteQuery,
    'mes devis': NlpIntent.quoteQuery,
  };
  ...
  static final Map<NlpIntent, Map<String, double>> _intentBags = {
    ...
    NlpIntent.actionNewQuote: {
      'devis': 1.8, 'proforma': 1.8, 'facture proforma': 1.8, 'estimation': 1.5,
      'nouveau': 0.6, 'creer': 0.6, 'ajouter': 0.5, 'faire': 0.6,
      'nouveau devis': 2.0, 'creer devis': 2.0,
    },
    NlpIntent.quoteQuery: {
      'devis': 1.2, 'proforma': 1.2, 'facture proforma': 1.2,
      'liste': 1.0, 'recherche': 0.8, 'voir': 0.7, 'afficher': 0.7,
      'combien': 0.8, 'nombre': 0.8,
    },
  ...
  static String getPhoneticKey(String input) {
    ...
    // Remplacements phonétiques adaptés au français/bambara
    s = s.replaceAll('ph', 'f');
    s = s.replaceAll('th', 't');
    s = s.replaceAll('kh', 'k');
    s = s.replaceAll('qu', 'k');
    ...
    s = s.replaceAll('ou', 'u');
    s = s.replaceAll('w', 'v');
    s = s.replaceAll('u', 'v'); // Rapproche u et v
    s = s.replaceAll('y', 'i');
    
    // Consonnes proches
    s = s.replaceAll('dj', 'j');
    s = s.replaceAll('c', 's'); // Simplifie c en s
    s = s.replaceAll('z', 's'); // Rapproche z et s
    s = s.replaceAll('h', '');  // Muet
    
    // Simplification des doubles consonnes
    final doubleConsonants = ['ss', 'tt', 'rr', 'pp', 'mm', 'nn', 'll', 'dd', 'ff', 'bb', 'gg', 'kk'];
    for (final dc in doubleConsonants) {
      s = s.replaceAll(dc, dc[0]);
    }
    
    // Suppression des lettres muettes finales typiques
    if (s.endsWith('s') && s.length > 3) s = s.substring(0, s.length - 1);
    if (s.endsWith('t') && s.length > 3) s = s.substring(0, s.length - 1);
    if (s.endsWith('d') && s.length > 3) s = s.substring(0, s.length - 1);
    if (s.endsWith('e') && s.length > 3) s = s.substring(0, s.length - 1);
    
    return s;
  }

  static double phoneticSimilarity(String a, String b) {
    final keyA = getPhoneticKey(a);
    final keyB = getPhoneticKey(b);
    if (keyA.isEmpty && keyB.isEmpty) return 1.0;
    if (keyA == keyB) return 1.0;
    return similarity(keyA, keyB);
  }
  ...
  static NlpResult analyze(String input, {AssistantContext context = AssistantContext.general}) {
    ...
    for (final intentEntry in _intentBags.entries) {
      ...
      if (isQuestion && (isActIntent || isNavIntent)) {
        multiplier *= 0.1; // Huge penalty for actions/nav if the user is asking a question
      } else {
      ...
      if (hits > 0) {
        ...
        if (maxWeight >= 1.5) {
          scores[intent] = maxWeight * 0.9 * multiplier; 
        } else {
          final meaningfulWords = words.where((w) => !_stopWords.contains(w)).toList();
          scores[intent] = ((score * multiplier) / max(1, meaningfulWords.length)) * (1.0 + hits * 0.5);
        }
      }
    }
