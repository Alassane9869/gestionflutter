...
class HorizonEngine {
  ...
  List<HorizonPrediction> _checkSurvivalGuard(List<Product> products) {
    ...
    for (final entry in byCategory.entries) {
      ...
      if (avgMargin < 10) {
        final isLoss = avgMargin < 0;
        insights.add(HorizonPrediction(
          predictedValue: avgMargin,
          trend: "SURVIE",
          key: "survie_${entry.key}",
          suggestion: isLoss
              ? "⚠️ Alerte Critique : La catégorie '${entry.key}' est VENDUE À PERTE en moyenne (marge de ${avgMargin.toStringAsFixed(1)}%). Réajustez vos prix immédiatement !"
              : "⚠️ Alerte Survie : La catégorie '${entry.key}' a une marge moyenne faible de ${avgMargin.toStringAsFixed(1)}%. Réajustez vos prix !",
        ));
      }
    }
    return insights;
