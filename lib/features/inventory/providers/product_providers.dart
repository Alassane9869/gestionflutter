...
class ProductListNotifier extends AsyncNotifier<List<Product>> {
  ...
  Future<void> addProduct(Product product, {String? warehouseId, String? reason}) async {
    state = const AsyncLoading();
    await ref.read(productRepositoryProvider).insert(product, warehouseId: warehouseId);

    // Historisation du stock initial si > 0 (uniquement pour les produits physiques)
    if (!product.isService && product.quantity > 0) {
      ...
      final movement = StockMovement(
        ...
        reason: reason ?? "Création du produit / Stock initial",
        userId: userId,
  ...
  Future<void> updateProduct(Product product) async {
    ...
    if (oldProduct != null && !product.isService && oldProduct.quantity != product.quantity) {
...
final productsSearchQueryProvider = NotifierProvider<ProductsSearchQueryNotifier, String>(ProductsSearchQueryNotifier.new);

class ProductsStockFilterNotifier extends Notifier<String> {
  @override
  String build() => 'all';
  void update(String val) => state = val;
}
final productsStockFilterProvider = NotifierProvider<ProductsStockFilterNotifier, String>(ProductsStockFilterNotifier.new);
