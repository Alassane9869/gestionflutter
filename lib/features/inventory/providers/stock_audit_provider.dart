...
class StockAuditActions {
  ...
  Future<String> startNewAudit({String? notes, String? category}) async {
    ...
    await db.transaction((txn) async {
      ...
      // Snapshot products with optional category filter (excluding services)
      final List<Map<String, dynamic>> productMaps = category != null && category.isNotEmpty
          ? await txn.query('products', where: 'category = ? AND is_service = 0', whereArgs: [category])
          : await txn.query('products', where: 'is_service = 0');
