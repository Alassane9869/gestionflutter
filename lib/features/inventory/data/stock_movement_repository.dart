...
final stockMovementRepositoryProvider = Provider<StockMovementRepository>((ref) {
  return StockMovementRepository(ref.watch(databaseServiceProvider), ref);
});
...
class StockMovementRepository {
  ...
  StockMovementRepository(this._dbService, this._ref);
  ...
  Future<void> insert(StockMovement movement) async {
    ...
    if (settings?.networkMode == NetworkMode.client) {
      _ref.read(clientSyncProvider).sendStockMovementToServer(updatedMovement);
      // pas besoin de syncPendingAuditData() ici si c'est fait par les autres repos pour éviter le spam, 
      // ou on peut le laisser si on veut que ce soit immédiat.
    }
  }
