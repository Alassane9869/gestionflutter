...
class ProductRepository {
  ...
  Future<void> update(Product product) async {
    ...
    final settings = _ref.read(shopSettingsProvider).value;
    if (settings?.networkMode == NetworkMode.client) {
      _ref.read(clientSyncProvider).sendProductToServer(product);
      _ref.read(clientSyncProvider).syncPendingAuditData();
    }
  }
  ...
  Future<void> delete(String id) async {
    ...
    final settings = _ref.read(shopSettingsProvider).value;
    if (settings?.networkMode == NetworkMode.client) {
      // Pour une suppression on synchronise pour que le serveur sache.
      // (Si l'API n'a pas de route delete, la synchro audit s'en occupera)
      _ref.read(clientSyncProvider).syncPendingAuditData();
    }
  }
  ...
  Future<void> archive(String id) async {
    ...
    final settings = _ref.read(shopSettingsProvider).value;
    if (settings?.networkMode == NetworkMode.client) {
      final archivedProduct = product.copyWith(isActive: false);
      _ref.read(clientSyncProvider).sendProductToServer(archivedProduct);
      _ref.read(clientSyncProvider).syncPendingAuditData();
    }
  }
