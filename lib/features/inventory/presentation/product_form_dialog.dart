...
class _ProductFormDialogState extends ConsumerState<ProductFormDialog> {
  ...
  static const List<String> _standardUnits = [
    ...
    'Lot', 'Ensemble', 'Rouleau', 'Bidon', 'Sachet', 'Baril', 'Tonne'
  ];
    ...
    if (mounted) {
      setState(() {
        final defaultCategories = [
          ...
          'Vêtements', 'Chaussures', 'Alimentation', 'Boissons',
          'Cosmétiques', 'Mobilier', 'Quincaillerie', 'Services', 'Autre'
        ];
        _existingCategories = categories.isNotEmpty ? categories : defaultCategories;
        // Merge them and keep unique
        if (categories.isNotEmpty) {
           _existingCategories = {...categories, ...defaultCategories}.toList();
        }
      });
    }
  }
    ...
    if (sellingPrice > 0 && purchasePrice > 0 && sellingPrice < purchasePrice) {
      showDialog(
        ...
        builder: (context) => EnterpriseWidgets.buildPremiumDialog(
          ...
          width: 400,
          child: Text(
            "Le prix de vente (${sellingPrice.toStringAsFixed(0)}) est inférieur au prix d'achat (${purchasePrice.toStringAsFixed(0)}).\n\nÊtes-vous sûr de vouloir enregistrer ce produit avec une marge négative ?",
    ...
    return EnterpriseWidgets.buildPremiumDialog(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              if (useWideLayout)
                _buildWideLayout(context, settings, currency)
              else
                _buildCompactLayout(context, settings, currency),
            ],
          ),
        ),
      ),
      actions: [
        ...
        FilledButton.icon(
          ...
          label: Text(isEditing ? "Enregistrer" : "Ajouter au stock", style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold)),
  ...
  Widget _buildWideLayout(BuildContext context, dynamic settings, String currency) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // Left Column: Identification and notes
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: OutlinedButton(
                          ...
                          child: const Text("PHOTO", style: TextStyle(fontSize: 9, fontWeight: FontWeight.w900)),
                        ),
                      ),
                    ],
                  ),
        ...
        // Right Column: Finances and Logistics/Stock
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _buildSectionHeader("Tarification & Finances", FluentIcons.money_hand_24_regular),
              Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Expanded(
                          ...
                          child: Row(
                            children: [
                              Icon(isLoss ? FluentIcons.warning_24_regular : FluentIcons.checkmark_circle_24_regular, 
                                color: isLoss ? Colors.red : Colors.green, size: 18),
                              const SizedBox(width: 10),
                              Expanded(
                                child: Text(
                                  isLoss 
                                    ? "ALERTE : Vente à perte ! Marge: ${margin.toStringAsFixed(0)} $currency"
                                    : "MARGE BRUTE : +${margin.toStringAsFixed(0)} $currency par unité.",
                                  style: TextStyle(
  ...
  Widget _buildCompactLayout(BuildContext context, dynamic settings, String currency) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildSectionHeader("Identification de l'article", FluentIcons.contact_card_group_24_regular),
