...
class InventoryAutomationService {
  ...
  Future<void> printBarcodeLabels(List<Product> products) async {
    ...
    if (settings.labelFormat == LabelPrintingFormat.a4Sheets) {
      ...
      pdf.addPage(pw.MultiPage(
          ...
          build: (context) => rows.map((row) => pw.Padding(
                padding: const pw.EdgeInsets.only(bottom: 5),
                child: pw.Row(children: [
                  ...row.map((p) => pw.Expanded(
                        child: pw.Padding(
  ...
  static pw.Widget _buildLabelWidget(Product p, ShopSettings settings, {required bool isVertical, bool compact = false}) {
    ...
    // Safety fallback: if the data is not valid for the chosen barcode format, fall back to standard Code 128 (highly permissive)
    bool isBarcodeValid = barcodeType.isValid(finalData);
    if (!isBarcodeValid) {
      barcodeType = Barcode.code128();
      finalData = barcodeData;
      isBarcodeValid = barcodeType.isValid(finalData);
    }
    ...
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          ...
          pw.SizedBox(
            height: labelHeightPts * (compact ? 0.45 : 0.4),
            child: isBarcodeValid
                ? pw.BarcodeWidget(
                    barcode: barcodeType, 
                    data: finalData, 
                    width: double.infinity, 
                ...
                : pw.Container(
                    ...
                    child: pw.Text(
                      ...
                      style: pw.TextStyle(fontSize: titleSize * 0.5, color: PdfColors.red800, fontWeight: pw.FontWeight.bold),
                    ),
                  ),
          ),
