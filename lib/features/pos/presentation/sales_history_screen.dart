...
class _SalesHistoryScreenState extends ConsumerState<SalesHistoryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    _searchQuery = ref.watch(salesSearchQueryProvider);
    _statusFilter = ref.watch(salesStatusFilterProvider);
    _paymentFilter = ref.watch(salesPaymentFilterProvider);
            ...
            child: Row(children: [
              ...
              Expanded(
                ...
                child: TextField(
                  ...
                  onChanged: (v) => ref.read(salesSearchQueryProvider.notifier).update(v),
              ...
              ...[
                ...
                color: f.$3,
                onTap: () => ref.read(salesStatusFilterProvider.notifier).update(f.$1),
              )),
              _VertDivider(isDark: isDark),
              // Payment filter
              PopupMenuButton<String>(
                tooltip: "Filtrer par méthode de paiement",
                onSelected: (v) => ref.read(salesPaymentFilterProvider.notifier).update(v),
                                    ...
                                    if (!isRefunded)
                                      PopupMenuButton<String>(
                                        ...
                                        itemBuilder: (_) => [
                                          ...
                                          const PopupMenuItem(value: 'print_ticket', child: Row(children: [
                                            Icon(FluentIcons.receipt_24_regular, size: 18),
                                            SizedBox(width: 10),
                                            Text("Imprimer Ticket"),
                                          ])),
                                          const PopupMenuItem(value: 'print_invoice', child: Row(children: [
                                            Icon(FluentIcons.document_pdf_24_regular, size: 18),
                                            SizedBox(width: 10),
                                            Text("Imprimer Facture / A4"),
                                          ])),
                                          // SÉCURITÉ : Retour visible uniquement si l'utilisateur a la permission canRefund
                                          if (user?.canRefund ?? false)
                                        ...
                                        onSelected: (v) {
                                          ...
                                          if (v == 'print_ticket') _printSaleDocument(context, sd, 'ticket');
                                          if (v == 'print_invoice') _printSaleDocument(context, sd, 'invoice');
                                          if (v == 'return') {
                                            // Garde de sécurité supplémentaire
                                            if (!(user?.canRefund ?? false)) {
                                              ScaffoldMessenger.of(context).showSnackBar(
                                                const SnackBar(content: Text("Permission refusée : Retour/Annulation non autorisé."), backgroundColor: Colors.red),
                                              );
                                              return;
                                            }
                                            showDialog(context: context, builder: (_) => ReturnSaleDialog(saleData: sd));
                                          }
                                        },
                                      )
                                    else
  ...
  void _printSaleDocument(BuildContext context, SaleWithDetails sd, String docType) async {
    ...
    if (settings == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Paramètres du magasin non chargés.")),
      );
      return;
    }
    ...
    // Get client details from provider cache
