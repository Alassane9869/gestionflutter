...
class QuoteRepository {
  ...
  Future<List<Map<String, dynamic>>> getAllQuotes({String? userId}) async {
    final db = await _dbService.database;
    final quotes = userId != null 
        ? await db.query('quotes', where: 'user_id = ?', whereArgs: [userId], orderBy: 'date DESC')
        : await db.query('quotes', orderBy: 'date DESC');
...
final quoteListProvider = FutureProvider<List<Map<String, dynamic>>>((ref) {
  ...
  // SÉCURITÉ : Filtrer les devis par utilisateur pour les rôles non-globaux
  final isGlobal = user?.canViewGlobalSalesHistory ?? false;
  final userId = isGlobal ? null : user?.id;
  return ref.watch(quoteRepositoryProvider).getAllQuotes(userId: userId);
});
