...
class CartNotifier extends Notifier<List<PosCartItem>> {
        ...
        state = [
          for (final item in state)
            if (item.productId == p.id)
              item.copyWith(qty: p.isService ? item.qty + 1.0 : (item.qty + 1.0 > p.quantity ? p.quantity : item.qty + 1.0))
            else
              item,
        ];
      }
  ...
  void loadFromQuote(List<dynamic> quoteItems) {
    state = quoteItems.map((i) {
      final double price = (i['unit_price'] as num).toDouble();
      final double qty = (i['quantity'] as num).toDouble();
      final double discountAmt = (i['discount_amount'] as num? ?? 0.0).toDouble();
      final double discountPct = (price * qty > 0) ? (discountAmt / (price * qty)) * 100 : 0.0;
      return PosCartItem(
    ...
    try {
      ...
      await db.transaction((txn) async {
        ...
        for (var item in cart) {
          ...
          String? unit;
          bool productIsService = false;

          if (productId != null && productId != 'custom') {
            final pRow = await txn.query(
              'products',
              columns: ['id', 'weighted_average_cost', 'purchasePrice', 'unit', 'is_service'],
              where: 'id = ?',
              whereArgs: [productId],
            );
            if (pRow.isNotEmpty) {
              ...
              unit = pRow.first['unit'] as String?;
              productIsService = (pRow.first['is_service'] as int? ?? 0) == 1;
            }
          }
          final isCustom = !exists || productId == null || productId == 'custom';
          
          final String name = item['name'] as String? ?? 'Article';
    ...
    try {
      ...
      await db.transaction((txn) async {
        ...
        for (var entry in returnedQuantities.entries) {
          ...
          // Ne mettre à jour le stock que si c'est un vrai produit (et pas un service)
          if (productId != null) {
            final pRow = await txn.query('products', columns: ['is_service'], where: 'id = ?', whereArgs: [productId]);
            final isService = pRow.isNotEmpty && (pRow.first['is_service'] as int? ?? 0) == 1;
        ...
        if (refundAmount > 0) {
          ...
          // Update sale record
          await txn.rawUpdate(
            'UPDATE sales SET status = ?, refunded_amount = ?, credit_amount = credit_amount - ?, refunded_at = ?, refunded_by_user_id = ? WHERE id = ?',
            [status, newRefunded, amountToCancelDebt, refundDate.toIso8601String(), user.id, saleId],
          );
