...
class QuoteService {
  ...
  static QuoteData _sanitizeData(QuoteData data) {
    final cleanItems = data.items.map((item) => QuoteItem(
      name: _c(item.name),
      qty: item.qty,
  ...
  static Future<pw.Document> _buildMinimaliste(QuoteData data) async {
    ...
    final slateAccent = PdfColor.fromHex('#475569'); // Ardoise chic
    ...
    doc.addPage(pw.MultiPage(
      ...
      build: (ctx) => [
        // En-tête minimaliste et épuré (pas de bandeau lourd)
        pw.Row(
          ...
          children: [
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                _buildAdaptiveLogo(data, maxWidth: 90, maxHeight: 50, margin: const pw.EdgeInsets.only(bottom: 8)),
                pw.Text(data.settings.name.toUpperCase(), style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold, color: slateAccent, letterSpacing: 1)),
            ...
            pw.Column(
              ...
              children: [
                pw.Text(data.documentTitle.toUpperCase(), style: pw.TextStyle(fontSize: 22, fontWeight: pw.FontWeight.normal, color: slateAccent, letterSpacing: 3)),
                pw.SizedBox(height: 4),
                pw.Text('DEVIS N° ${data.quoteNumber}', style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold, color: PdfColors.black)),
                pw.Text('Date d\'émission : ${DateFormatter.formatDate(data.date)}', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey600)),
              ],
            ),
          ],
        ),
        pw.SizedBox(height: 20),
        pw.Divider(color: slateAccent, thickness: 0.5),
          ...
          children: [
            pw.Expanded(
              child: pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  pw.Text('DESTINATAIRE', style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold, color: slateAccent, letterSpacing: 1.2)),
                  pw.SizedBox(height: 4),
                  pw.Text(data.clientName ?? 'Client Potentiel', style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)),
                  if (data.clientPhone != null) pw.Text('Tél : ${data.clientPhone}', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
                  if (data.clientAddress != null) pw.Text(data.clientAddress!, style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
                ],
              ),
            ),
            pw.Expanded(
              child: pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.end,
                children: [
                  pw.Text('VALIDITÉ DU DEVIS', style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold, color: slateAccent, letterSpacing: 1.2)),
                  pw.SizedBox(height: 4),
                  pw.Text('Valable ${data.settings.quoteValidityDays} jours', style: const pw.TextStyle(fontSize: 9)),
                  pw.Text('Date limite : ${data.validUntil != null ? DateFormatter.formatDate(data.validUntil!) : "30 jours"}', style: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold, color: PdfColors.red800)),
                ],
              ),
            ),
          ],
        ...
        pw.SizedBox(height: 20),
        // Tableau d'articles minimaliste
        pw.Table(
          columnWidths: {0: const pw.FlexColumnWidth(5), 1: const pw.FlexColumnWidth(1.5), 2: const pw.FlexColumnWidth(2), 3: const pw.FlexColumnWidth(2)},
          children: [
            pw.TableRow(
              decoration: pw.BoxDecoration(color: PdfColor.fromHex('#F1F5F9')),
              children: [
                _pCell('DÉSIGNATION / PRESTATION', isHeader: false, bold: true, color: slateAccent),
                _pCell('QTÉ', isHeader: false, align: pw.TextAlign.center, bold: true, color: slateAccent),
                _pCell('P. UNITAIRE', isHeader: false, align: pw.TextAlign.right, bold: true, color: slateAccent),
                _pCell('TOTAL HT', isHeader: false, align: pw.TextAlign.right, bold: true, color: slateAccent),
              ],
            ),
            ...data.items.asMap().entries.map((e) => pw.TableRow(
              decoration: pw.BoxDecoration(color: e.key % 2 == 0 ? PdfColors.white : PdfColor.fromHex('#F8FAFC')),
              children: [
                pw.Padding(
                  ...
                  child: pw.Column(
                    ...
                    children: [
                      ...
                      if (e.value.description != null) pw.Text(e.value.description!, style: const pw.TextStyle(fontSize: 7.5, color: PdfColors.grey600)),
                    ],
                  ),
                ),
                _pCell("${DateFormatter.formatQuantity(e.value.qty)}${e.value.unit != null ? " ${e.value.unit}" : ""}", align: pw.TextAlign.center),
                _pCell(fmt(e.value.unitPrice), align: pw.TextAlign.right),
                _pCell(fmt(e.value.lineTotal), align: pw.TextAlign.right, bold: true, color: slateAccent),
              ],
            )),
          ],
        ),
            ...
            pw.Container(
              ...
              child: pw.Column(
                children: [
                  ...
                  pw.Divider(color: slateAccent, thickness: 0.5),
                  _pTotalRow('MONTANT TOTAL TTC', fmt(data.totalAmount), isBig: true, color: slateAccent),
                  if (data.shouldShowTax && !data.shouldBeDetailed)
        ...
        pw.Row(
          ...
          children: [
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                pw.Text("BON POUR ACCORD LE CLIENT", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: slateAccent)),
                pw.Text("Mention manuscrite 'Bon pour accord' + signature", style: const pw.TextStyle(fontSize: 7, color: PdfColors.grey500)),
            ...
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.end,
              children: [
                pw.Text("POUR LA SOCIÉTÉ", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: slateAccent)),
    ...
    doc.addPage(pw.MultiPage(
      pageTheme: pw.PageTheme(
        ...
        buildBackground: (ctx) => pw.FullPage(
          ...
          child: pw.Stack(
            children: [
              pw.Positioned(left: 0, top: 0, right: 0, child: pw.Container(height: 5, color: amethystColor)),
              pw.Positioned(left: 0, bottom: 0, right: 0, child: pw.Container(height: 5, color: goldAccent)),
            ],
          ),
        ),
      ),
        ...
        pw.Row(
          ...
          children: [
            pw.Column(
              ...
              children: [
                _buildAdaptiveLogo(data, maxWidth: 90, maxHeight: 50, margin: const pw.EdgeInsets.only(bottom: 8)),
                pw.Text(data.settings.name.toUpperCase(), style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold, color: amethystColor, letterSpacing: 1.5)),
                if (data.settings.slogan.isNotEmpty) pw.Text(data.settings.slogan, style: pw.TextStyle(fontSize: 8, fontStyle: pw.FontStyle.italic, color: goldAccent)),
                if (data.settings.address.isNotEmpty) pw.Text(data.settings.address, style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
              ],
            ),
            pw.Column(
              ...
              children: [
                pw.Container(
                  ...
                  child: pw.Text(data.documentTitle.toUpperCase(), style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold, color: amethystColor, letterSpacing: 2)),
                ),
                pw.SizedBox(height: 4),
                pw.Text('N° ${data.quoteNumber}', style: pw.TextStyle(fontSize: 11, fontWeight: pw.FontWeight.bold, color: amethystColor)),
                pw.Text('Date : ${DateFormatter.formatDate(data.date)}', style: const pw.TextStyle(fontSize: 8)),
              ],
            ),
          ],
        ),
            ...
            pw.Expanded(
              child: pw.Container(
                ...
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text('PROPOSITION PRÉPARÉE POUR', style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold, color: amethystColor, letterSpacing: 1)),
                    pw.SizedBox(height: 4),
                    pw.Text(data.clientName ?? 'Client Potentiel', style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold, color: amethystColor)),
                    if (data.clientPhone != null) pw.Text('Contact : ${data.clientPhone}', style: const pw.TextStyle(fontSize: 8)),
                    if (data.clientAddress != null) pw.Text(data.clientAddress!, style: const pw.TextStyle(fontSize: 8)),
                  ],
                ),
              ),
            ),
            ...
            pw.Expanded(
              child: pw.Container(
                ...
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text('PÉRIODE DE VALIDITÉ', style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold, color: goldAccent, letterSpacing: 1)),
                    pw.SizedBox(height: 4),
                    pw.Text('Valable jusqu\'au : ${data.validUntil != null ? DateFormatter.formatLongDate(data.validUntil!) : "30 jours"}', style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold, color: goldAccent)),
                    pw.Text('Validité standard : ${data.settings.quoteValidityDays} jours', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey600)),
                  ],
                ),
              ),
            ),
          ...
          children: [
            pw.TableRow(
              ...
              children: [
                ...
                _pCell('P. UNITAIRE', isHeader: true, align: pw.TextAlign.right),
                _pCell('MONTANT HT', isHeader: true, align: pw.TextAlign.right),
              ],
            ),
            ...data.items.asMap().entries.map((e) => pw.TableRow(
              decoration: pw.BoxDecoration(
                color: e.key % 2 == 0 ? PdfColors.white : PdfColor.fromHex('#F5F3FF'),
                border: pw.Border(bottom: pw.BorderSide(color: PdfColor.fromHex('#EDE9FE'), width: 0.5)),
              ),
              children: [
                pw.Padding(
                  padding: const pw.EdgeInsets.all(8),
                  child: pw.Column(
                    crossAxisAlignment: pw.CrossAxisAlignment.start,
                    children: [
                      pw.Text(e.value.name, style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 9.5)),
                      if (e.value.description != null) pw.Text(e.value.description!, style: const pw.TextStyle(fontSize: 7.5, color: PdfColors.grey600)),
                    ],
                  ),
                ),
                _pCell("${DateFormatter.formatQuantity(e.value.qty)}${e.value.unit != null ? " ${e.value.unit}" : ""}", align: pw.TextAlign.center),
                _pCell(fmt(e.value.unitPrice), align: pw.TextAlign.right),
                _pCell(fmt(e.value.lineTotal), align: pw.TextAlign.right, bold: true, color: amethystColor),
              ],
            )),
          ],
        ),
            ...
            pw.Container(
              ...
              child: pw.Column(
                children: [
                  ...
                  _pTotalRow(data.settings.labelTTC, fmt(data.totalAmount), isBig: true, color: amethystColor),
                  if (data.shouldShowTax && !data.shouldBeDetailed)
                    _pTotalRow('Dont ${data.settings.taxName}', fmt(data.taxAmount)),
                ],
              ),
        ...
        pw.Row(
          ...
          children: [
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                pw.Text("ACCORD CLIENT", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: amethystColor)),
                pw.Text("Date, signature et mention manuscrite", style: const pw.TextStyle(fontSize: 7, color: PdfColors.grey500)),
                pw.SizedBox(height: 35),
            ...
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.end,
              children: [
                pw.Text("POUR LA SOCIÉTÉ", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: goldAccent)),
                pw.SizedBox(height: 42),
          ...
          children: [
            pw.Expanded(
              child: pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  pw.Text("BON POUR ACCORD LE CLIENT", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: corporateBlue)),
                  pw.Text("Signature + mention manuscrite", style: const pw.TextStyle(fontSize: 7, color: PdfColors.grey500)),
                  pw.SizedBox(height: 35),
                  pw.Container(width: 140, height: 0.5, color: PdfColors.grey400),
                ],
              ),
        ...
        pw.Row(
          ...
          children: [
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.end,
              children: [
                pw.Text("POUR LA SOCIÉTÉ", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: corporateBlue)),
                pw.SizedBox(height: 35),
                pw.Container(width: 140, height: 0.5, color: PdfColors.grey400),
              ],
            ),
          ...
          pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
            ...
            pw.Container(padding: const pw.EdgeInsets.symmetric(horizontal: 18, vertical: 10), color: modernGreen,
              child: pw.Column(children: [
                pw.Text(data.documentTitle.toUpperCase(), style: pw.TextStyle(fontSize: 22, fontWeight: pw.FontWeight.bold, color: PdfColors.white)),
                ...
                children: [
                  pw.Padding(
                    ...
                    child: pw.Column(
                      crossAxisAlignment: pw.CrossAxisAlignment.start,
                      children: [
                        pw.Text(e.value.name, style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
                        if (e.value.description != null) pw.Text(e.value.description!, style: const pw.TextStyle(fontSize: 7.5, color: PdfColors.grey600)),
                      ],
                    ),
                  ),
            ...
            children: [
              pw.Expanded(
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text("ACCORD DU CLIENT", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: modernGreen)),
                    pw.Text("Signature + mention manuscrite", style: const pw.TextStyle(fontSize: 7, color: PdfColors.grey500)),
                    pw.SizedBox(height: 35),
                    pw.Container(width: 140, height: 0.5, color: PdfColors.grey400),
                  ],
          ...
          pw.Row(
            ...
            children: [
              pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.end,
                children: [
                  pw.Text("POUR LA SOCIÉTÉ", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: modernGreen)),
                  pw.SizedBox(height: 35),
                  pw.Container(width: 140, height: 0.5, color: PdfColors.grey400),
                ],
              ),
    ...
    doc.addPage(pw.MultiPage(
      ...
      build: (ctx) => [
        ...
        pw.Row(
          ...
          children: [
            pw.Column(
              ...
              children: [
                _buildAdaptiveLogo(data, maxWidth: 90, maxHeight: 50, margin: const pw.EdgeInsets.only(bottom: 8)),
                pw.Text(data.settings.name.toUpperCase(), style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold, color: cleanAccent)),
                if (data.settings.slogan.isNotEmpty) pw.Text(data.settings.slogan, style: pw.TextStyle(fontSize: 8, fontStyle: pw.FontStyle.italic, color: PdfColors.grey600)),
                if (data.settings.address.isNotEmpty) pw.Text(data.settings.address, style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
                if (data.settings.phone.isNotEmpty) pw.Text('Tél: ${data.settings.phone}', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
              ],
            ),
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.end,
              children: [
                pw.Text(data.documentTitle.toUpperCase(), style: pw.TextStyle(fontSize: 22, fontWeight: pw.FontWeight.bold, color: cleanAccent, letterSpacing: 2)),
                pw.SizedBox(height: 4),
                pw.Text('N° ${data.quoteNumber}', style: pw.TextStyle(fontSize: 11, fontWeight: pw.FontWeight.bold, color: PdfColors.black)),
                ...
                if (data.validUntil != null)
                  pw.Text('Valide jusqu\'au : ${DateFormatter.formatDate(data.validUntil!)}', style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: PdfColors.red700)),
              ],
            ),
          ],
        ),
          ...
          children: [
            pw.Expanded(
              child: pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  pw.Text('PROPOSITION DESTINÉE À :', style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: cleanAccent)),
                  pw.SizedBox(height: 4),
                  pw.Text(data.clientName ?? 'Client Potentiel', style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)),
                  if (data.clientPhone != null) pw.Text('Tél : ${data.clientPhone}', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
                  if (data.clientAddress != null) pw.Text(data.clientAddress!, style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
                  if (data.clientEmail != null) pw.Text('Email : ${data.clientEmail}', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
                ],
              ),
            ),
            pw.Expanded(
              child: pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.end,
                children: [
                  pw.Text('INFORMATIONS :', style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: cleanAccent)),
                  pw.SizedBox(height: 4),
                  pw.Text('Validité : ${data.settings.quoteValidityDays} jours', style: const pw.TextStyle(fontSize: 9)),
                  pw.Text('Agent commercial : ${data.cashierName}', style: const pw.TextStyle(fontSize: 9, color: PdfColors.grey700)),
                ],
              ),
            ),
          ],
        ...
        pw.Table(
          ...
          children: [
            pw.TableRow(
              decoration: pw.BoxDecoration(border: pw.Border(bottom: pw.BorderSide(color: cleanAccent, width: 1))),
              children: [
                _pCell('DÉSIGNATION / PRESTATION', isHeader: false, bold: true, color: cleanAccent),
                _pCell('QTÉ', isHeader: false, align: pw.TextAlign.center, bold: true, color: cleanAccent),
                _pCell('P. UNITAIRE', isHeader: false, align: pw.TextAlign.right, bold: true, color: cleanAccent),
                _pCell('TOTAL HT', isHeader: false, align: pw.TextAlign.right, bold: true, color: cleanAccent),
              ],
            ),
            ...data.items.asMap().entries.map((e) => pw.TableRow(
              ...
              children: [
                ...
                _pCell("${DateFormatter.formatQuantity(e.value.qty)}${e.value.unit != null ? " ${e.value.unit}" : ""}", align: pw.TextAlign.center),
                _pCell(fmt(e.value.unitPrice), align: pw.TextAlign.right),
                _pCell(fmt(e.value.lineTotal), align: pw.TextAlign.right, bold: true, color: cleanAccent),
              ],
            )),
          ],
        ),
            ...
            pw.Container(
              ...
              child: pw.Column(
                children: [
                  ...
                  _pTotalRow('MONTANT NET TTC', fmt(data.totalAmount), isBig: true, color: cleanAccent),
                  if (data.shouldShowTax && !data.shouldBeDetailed)
                    _pTotalRow('Dont ${data.settings.taxName}', fmt(data.taxAmount)),
                ],
              ),
        ...
        pw.Row(
          ...
          children: [
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                pw.Text("BON POUR ACCORD LE CLIENT", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: cleanAccent)),
                pw.Text("Mention manuscrite 'Bon pour accord' + signature", style: const pw.TextStyle(fontSize: 7, color: PdfColors.grey500)),
                pw.SizedBox(height: 35),
                pw.Container(width: 140, height: 0.5, color: PdfColors.grey400),
              ],
            ...
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.end,
              children: [
                pw.Text("POUR LA SOCIÉTÉ", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: cleanAccent)),
                pw.SizedBox(height: 42),
                pw.Container(width: 140, height: 0.5, color: PdfColors.grey400),
              ],
            ),
  ...
  static Future<pw.Document> _buildPrestige(QuoteData data) async {
    ...
    final primary = PdfColor.fromHex('#455A64'); // Blue Grey
    final secondary = PdfColor.fromHex('#00897B'); // Teal
    final lightBg = PdfColor.fromHex('#F5F7F9');
    final String currency = data.settings.currency;
    final bool removeDecimals = data.settings.removeDecimals;
    String fmt(double val) => DateFormatter.formatCurrency(val, currency, removeDecimals: removeDecimals);
    doc.addPage(pw.MultiPage(
      ...
      footer: (ctx) => pw.Padding(
        ...
        child: _buildInstitutionnelFooter(data),
      ),
      build: (ctx) => [
        pw.Container(
          height: 120,
          ...
          child: pw.Row(
            ...
            children: [
              pw.Column(
                ...
                children: [
                  _buildAdaptiveLogo(data, maxWidth: 90, maxHeight: 50, margin: const pw.EdgeInsets.only(bottom: 4)),
                  pw.Text(data.settings.name.toUpperCase(), style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold, color: PdfColors.white)),
                  if (data.settings.slogan.isNotEmpty) pw.Text(data.settings.slogan, style: pw.TextStyle(fontSize: 8, fontStyle: pw.FontStyle.italic, color: PdfColors.grey300)),
                ],
              ),
              pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.end,
                mainAxisAlignment: pw.MainAxisAlignment.center,
                children: [
                  pw.Text(data.documentTitle.toUpperCase(), style: pw.TextStyle(fontSize: 22, fontWeight: pw.FontWeight.bold, color: PdfColors.white, letterSpacing: 2)),
                  pw.SizedBox(height: 4),
                  pw.Text('Réf : ${data.quoteNumber}', style: pw.TextStyle(fontSize: 11, fontWeight: pw.FontWeight.bold, color: PdfColors.white)),
                  pw.Text('Date : ${DateFormatter.formatDate(data.date)}', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey300)),
                ],
              ),
            ],
          ),
        ...
        pw.Padding(
          padding: const pw.EdgeInsets.fromLTRB(30, 20, 30, 0),
          child: pw.Row(
            crossAxisAlignment: pw.CrossAxisAlignment.start,
            children: [
              pw.Expanded(
                ...
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text('DESTINATAIRE DE L\'OFFRE', style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: primary)),
                    pw.SizedBox(height: 4),
                    pw.Text(data.clientName ?? 'Client Potentiel', style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)),
                    if (data.clientPhone != null) pw.Text('Téléphone : ${data.clientPhone}', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
                    if (data.clientAddress != null) pw.Text('Adresse : ${data.clientAddress!}', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
                    if (data.clientEmail != null) pw.Text('Email : ${data.clientEmail}', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
                    pw.SizedBox(height: 20),
                    pw.Table(
                      columnWidths: {0: const pw.FlexColumnWidth(5), 1: const pw.FlexColumnWidth(1.5), 2: const pw.FlexColumnWidth(2), 3: const pw.FlexColumnWidth(2)},
                      children: [
                        pw.TableRow(
                          ...
                          children: [
                            ...
                            _pCell('QTÉ', isHeader: true, align: pw.TextAlign.center),
                            _pCell('P. UNITAIRE', isHeader: true, align: pw.TextAlign.right),
                            _pCell('TOTAL HT', isHeader: true, align: pw.TextAlign.right),
                          ],
                        ),
                        ...data.items.asMap().entries.map((e) => pw.TableRow(
                          decoration: pw.BoxDecoration(
                            color: e.key % 2 == 0 ? PdfColors.white : lightBg,
                          ...
                          children: [
                            pw.Padding(
                              ...
                              child: pw.Column(
                                crossAxisAlignment: pw.CrossAxisAlignment.start,
                                children: [
                                  pw.Text(e.value.name, style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 8.5)),
                                  if (e.value.description != null) pw.Text(e.value.description!, style: const pw.TextStyle(fontSize: 7.5, color: PdfColors.grey600)),
                                ],
                              ),
                            ),
                            _pCell("${DateFormatter.formatQuantity(e.value.qty)}${e.value.unit != null ? " ${e.value.unit}" : ""}", align: pw.TextAlign.center),
                            _pCell(fmt(e.value.unitPrice), align: pw.TextAlign.right),
                            _pCell(fmt(e.value.lineTotal), align: pw.TextAlign.right, bold: true, color: primary),
                          ],
                        )),
                      ],
                    ),
                        ...
                        pw.Container(
                          ...
                          child: pw.Column(
                            children: [
                              ...
                              _pTotalRow('Total Global TTC', fmt(data.totalAmount), isBig: true, color: secondary),
                              if (data.shouldShowTax && !data.shouldBeDetailed)
                                _pTotalRow('Dont ${data.settings.taxName}', fmt(data.taxAmount)),
                            ],
                          ),
              ...
              pw.Container(
                ...
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text('STATUT DEVIS', style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold, color: secondary)),
                    pw.SizedBox(height: 2),
                    pw.Text('EN ATTENTE', style: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold, color: primary)),
                    pw.Divider(color: PdfColors.grey300, height: 15),
                    pw.Text('VALIDITÉ', style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold, color: secondary)),
                    pw.SizedBox(height: 2),
                    pw.Text('Durée : ${data.settings.quoteValidityDays} jours', style: const pw.TextStyle(fontSize: 8)),
                    if (data.validUntil != null)
                      pw.Text('Limite : ${DateFormatter.formatDate(data.validUntil!)}', style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: PdfColors.red700)),
                    pw.Divider(color: PdfColors.grey300, height: 15),
                    pw.Text('AGENT COMMERCIAL', style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold, color: secondary)),
                    pw.SizedBox(height: 2),
                    pw.Text(data.cashierName, style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: primary)),
                    pw.SizedBox(height: 20),
                    pw.Text('ACCORD CLIENT :', style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold, color: primary)),
                    pw.SizedBox(height: 20),
                    pw.Container(height: 0.5, color: PdfColors.grey400),
                    pw.Text('Date & signature', style: const pw.TextStyle(fontSize: 6, color: PdfColors.grey600)),
                    pw.SizedBox(height: 20),
                    pw.Text('SOCIÉTÉ :', style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold, color: secondary)),
                    pw.SizedBox(height: 20),
                    pw.Container(height: 0.5, color: PdfColors.grey400),
                    pw.Text('Signature & cachet', style: const pw.TextStyle(fontSize: 6, color: PdfColors.grey600)),
                  ],
                ),
              ),
            ],
          ...
          children: [
            pw.Column(
              ...
              children: [
                ...
                pw.Text(data.settings.name.toUpperCase(), style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold, color: darkColor)),
                if (data.settings.slogan.isNotEmpty) pw.Text(data.settings.slogan, style: pw.TextStyle(fontSize: 8, fontStyle: pw.FontStyle.italic, color: PdfColors.grey600)),
                pw.Text(data.settings.address, style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
                pw.Text('Tél : ${data.settings.phone}', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
              ],
            ),
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.end,
              children: [
                pw.Container(
                  ...
                  child: pw.Text(data.documentTitle.toUpperCase(), style: pw.TextStyle(fontSize: 20, fontWeight: pw.FontWeight.bold, color: goldAccent, letterSpacing: 1.5)),
                ),
                pw.SizedBox(height: 6),
                pw.Text('DEVIS REFERENCE : ${data.quoteNumber}', style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold)),
                pw.Text('Émis le : ${DateFormatter.formatDate(data.date)}', style: const pw.TextStyle(fontSize: 8)),
                if (data.validUntil != null)
                  pw.Text('Valide jusqu\'au : ${DateFormatter.formatDate(data.validUntil!)}', style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: PdfColors.red700)),
              ],
            ),
            ...
            pw.Expanded(
              child: pw.Container(
                ...
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text('INFORMATIONS CLIENT', style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: darkColor)),
                    pw.SizedBox(height: 4),
                    pw.Text(data.clientName ?? 'Client Potentiel', style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)),
                    if (data.clientPhone != null) pw.Text('Contact Tél : ${data.clientPhone}', style: const pw.TextStyle(fontSize: 8)),
                    if (data.clientAddress != null) pw.Text('Adresse : ${data.clientAddress!}', style: const pw.TextStyle(fontSize: 8)),
                    if (data.clientEmail != null) pw.Text('Email : ${data.clientEmail}', style: const pw.TextStyle(fontSize: 8)),
                  ],
                ),
              ),
            ),
            ...
            pw.Expanded(
              child: pw.Container(
                ...
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text('CADRE COMMERCIAL', style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: darkColor)),
                    pw.SizedBox(height: 4),
                    pw.Text('Durée de validité : ${data.settings.quoteValidityDays} jours', style: const pw.TextStyle(fontSize: 9)),
                    pw.Text('Agent en charge : ${data.cashierName}', style: const pw.TextStyle(fontSize: 9)),
                    pw.Text('Statut : Proposition commerciale officielle', style: pw.TextStyle(fontSize: 8, fontStyle: pw.FontStyle.italic, color: PdfColors.grey600)),
                  ],
                ),
              ),
            ),
        ...
        pw.Table(
          ...
          children: [
            pw.TableRow(
              ...
              children: [
                _pCell('DÉSIGNATION / PRESTATION', isHeader: true, color: PdfColors.white),
                _pCell('QTÉ', isHeader: true, align: pw.TextAlign.center, color: PdfColors.white),
                _pCell('P. UNITAIRE', isHeader: true, align: pw.TextAlign.right, color: PdfColors.white),
                _pCell('MONTANT HT', isHeader: true, align: pw.TextAlign.right, color: PdfColors.white),
              ],
            ),
            ...data.items.asMap().entries.map((e) => pw.TableRow(
              decoration: pw.BoxDecoration(
                color: e.key % 2 == 0 ? PdfColors.white : PdfColor.fromHex('#F7F9FA'),
                border: pw.Border(bottom: pw.BorderSide(color: PdfColors.grey200, width: 0.5)),
              ),
              children: [
                pw.Padding(
        ...
        pw.Row(
          ...
          children: [
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                pw.Text("BON POUR ACCORD LE CLIENT", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: darkColor)),
                pw.Text("Signature précédée de la mention manuscrite 'Bon pour accord'", style: const pw.TextStyle(fontSize: 7, color: PdfColors.grey600)),
            ...
            pw.Container(
              ...
              child: pw.Column(
                children: [
                  ...
                  _pTotalRow('TOTAL NET A PAYER TTC', fmt(data.totalAmount), isBig: true, color: goldAccent),
                  if (data.shouldShowTax && !data.shouldBeDetailed)
                    _pTotalRow('Dont ${data.settings.taxName}', fmt(data.taxAmount)),
                ],
              ),
        ...
        pw.Row(
          ...
          children: [
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.end,
              children: [
                pw.Text("POUR LA SOCIÉTÉ (SIGNATURE & CACHET)", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold, color: darkColor)),
                pw.SizedBox(height: 40),
                pw.Container(width: 150, height: 0.5, color: darkColor),
              ],
            ),
        ...
        pw.Row(
          ...
          children: [
            pw.Column(
              ...
              children: [
                _buildAdaptiveLogo(data, maxWidth: 90, maxHeight: 50, margin: const pw.EdgeInsets.only(bottom: 8)),
                pw.Text(data.settings.name.toUpperCase(), style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
                if (data.settings.slogan.isNotEmpty) pw.Text(data.settings.slogan, style: pw.TextStyle(fontSize: 8, fontStyle: pw.FontStyle.italic)),
                pw.Text(data.settings.address, style: const pw.TextStyle(fontSize: 8)),
                pw.Text('Tél : ${data.settings.phone}', style: const pw.TextStyle(fontSize: 8)),
              ],
            ),
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.end,
              children: [
                pw.Text(data.documentTitle.toUpperCase(), style: pw.TextStyle(fontSize: 22, fontWeight: pw.FontWeight.bold)),
                pw.SizedBox(height: 4),
                pw.Text('Devis N° : ${data.quoteNumber}', style: pw.TextStyle(fontSize: 11, fontWeight: pw.FontWeight.bold)),
                pw.Text('Date : ${DateFormatter.formatDate(data.date)}', style: const pw.TextStyle(fontSize: 8)),
                if (data.validUntil != null)
                  pw.Text('Date limite : ${DateFormatter.formatDate(data.validUntil!)}', style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold)),
              ],
            ),
          ],
        ),
          ...
          children: [
            ...
            pw.Expanded(
              child: pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.end,
                children: [
                  pw.Text('INFORMATIONS SUPPLÉMENTAIRES :', style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold)),
                  pw.SizedBox(height: 4),
                  pw.Text('Période de validité : ${data.settings.quoteValidityDays} jours', style: const pw.TextStyle(fontSize: 8)),
                  pw.Text('Conseiller : ${data.cashierName}', style: const pw.TextStyle(fontSize: 8)),
                ],
              ),
            ),
          ],
        ...
        pw.Table(
          ...
          children: [
            pw.TableRow(
              ...
              children: [
                _pCell('DÉSIGNATION / PRESTATION', isHeader: false, bold: true, color: PdfColors.black),
                _pCell('QTÉ', isHeader: false, align: pw.TextAlign.center, bold: true, color: PdfColors.black),
                _pCell('P. UNITAIRE', isHeader: false, align: pw.TextAlign.right, bold: true, color: PdfColors.black),
                _pCell('TOTAL HT', isHeader: false, align: pw.TextAlign.right, bold: true, color: PdfColors.black),
              ],
            ),
            ...data.items.asMap().entries.map((e) => pw.TableRow(
              ...
              children: [
                ...
                _pCell("${DateFormatter.formatQuantity(e.value.qty)}${e.value.unit != null ? " ${e.value.unit}" : ""}", align: pw.TextAlign.center),
                _pCell(fmt(e.value.unitPrice), align: pw.TextAlign.right),
                _pCell(fmt(e.value.lineTotal), align: pw.TextAlign.right, bold: true, color: PdfColors.black),
              ],
            )),
          ],
        ),
        ...
        pw.Row(
          ...
          children: [
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                pw.Text("COMMANDE / ACCORD CLIENT", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold)),
                pw.Text("Mention manuscrite 'Bon pour accord' + signature", style: const pw.TextStyle(fontSize: 7, color: PdfColors.grey600)),
                pw.SizedBox(height: 35),
        ...
        pw.Row(
          ...
          children: [
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.end,
              children: [
                pw.Text("SIGNATURE POUR LA SOCIÉTÉ", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold)),
                pw.SizedBox(height: 35),
                pw.Container(width: 140, height: 0.5, color: PdfColors.black),
              ],
            ),
          ...
          children: [
            ...
            pw.Expanded(
              ...
              child: pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  pw.Text('VALIDITÉ DU DEVIS', style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold, color: accentColor)),
                  pw.SizedBox(height: 5),
                  pw.Text('Ce devis est une proposition commerciale valable ${s.quoteValidityDays} jours à compter de sa date d\'émission.', style: const pw.TextStyle(fontSize: 6.5, color: PdfColors.grey700)),
                  pw.Text('Les prix indiqués sont fermes et non révisables durant cette période.', style: const pw.TextStyle(fontSize: 6.5, color: PdfColors.grey700)),
                ],
              ),
            ),
            ...
            pw.Expanded(
              ...
              child: pw.Column(
                children: [
                  pw.Text('CACHE-T & SIGNATURE', style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold, color: PdfColors.grey500)),
                  pw.SizedBox(height: 35),
              ...
              ...data.items.asMap().entries.map((e) {
                ...
                return pw.TableRow(
                  ...
                  children: [
                    ...
                    _pCellSupreme("${DateFormatter.formatQuantity(item.qty)}${item.unit != null ? " ${item.unit}" : ""}", align: pw.TextAlign.center),
