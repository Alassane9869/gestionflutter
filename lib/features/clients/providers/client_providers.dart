...
class ClientListNotifier extends AsyncNotifier<List<Client>> {
  ...
  Future<void> deleteClient(String id) async {
    ...
    final settings = ref.read(shopSettingsProvider).value;
    if (settings?.networkMode == NetworkMode.client) {
      ref.read(clientSyncProvider).sendDeleteToServer('clients', id);
    }
  }
...
// Providers for Client screen filtering and sorting via voice/UI synchronization
class ClientsSearchQueryNotifier extends Notifier<String> {
  @override
  String build() => '';
  
  void update(String val) => state = val;
}
final clientsSearchQueryProvider = NotifierProvider<ClientsSearchQueryNotifier, String>(ClientsSearchQueryNotifier.new);

class ClientsFilterTabNotifier extends Notifier<String> {
  @override
  String build() => 'all';
  
  void update(String val) => state = val;
}
final clientsFilterTabProvider = NotifierProvider<ClientsFilterTabNotifier, String>(ClientsFilterTabNotifier.new);

class ClientsSortByNotifier extends Notifier<String> {
  @override
  String build() => 'name';
  
  void update(String val) => state = val;
}
final clientsSortByProvider = NotifierProvider<ClientsSortByNotifier, String>(ClientsSortByNotifier.new);
