...
// Provider pour l'historique des sessions fermées
// SÉCURITÉ : Filtré par utilisateur pour les rôles non-globaux
final closedSessionsProvider = FutureProvider<List<Map<String, dynamic>>>((ref) async {
  ...
  final isGlobal = user.canViewGlobalSalesHistory;
  
  if (isGlobal) {
    // Admin/Manager : voit toutes les sessions
    ...
    // Caissier : voit uniquement ses propres sessions
    return await db.rawQuery('''
      SELECT cs.*, u.username 
      FROM cash_sessions cs 
      LEFT JOIN users u ON cs.user_id = u.id
      WHERE cs.status = 'CLOSED' AND cs.user_id = ?
      ORDER BY cs.close_date DESC
      LIMIT 50
    ''', [user.id]);
  }
...
class SessionService {
  ...
  Future<CashSession?> openSession(double initialBalance, {bool forceClose = false}) async {
    ...
    // Vérifier s'il y a des sessions ouvertes d'autres utilisateurs
    ...
    if (openSessions.isNotEmpty) {
      for (final orphan in openSessions) {
        ...
        if (sessionAge.inHours >= 24) {
          ...
          if (forceClose) {
            await _autoCloseStaleSession(orphanSession);
            debugPrint("Session fermée de force: ${orphanSession.id} (${orphan['user_id']})");
          } else {
            final userRow = await db.query('users', columns: ['username'], where: 'id = ?', whereArgs: [orphan['user_id']]);
            final ownerName = userRow.isNotEmpty ? userRow.first['username'] as String : "un autre utilisateur";
