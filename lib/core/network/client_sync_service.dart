...
class ClientSyncService {
  ...
  static const int _maxReconnectDelay = 120; // seconds
  
  // ⚡ P1 FIX (M4): Debounce global de syncPendingAuditData
  DateTime? _lastAuditSync;
  static const Duration _auditSyncDebounce = Duration(seconds: 30);
  
  // ⚡ P1 FIX (C4): Timeout HTTP standard
  ...
  String _getBaseUrl() {
    ...
    // ⚡ Vérifier qu'on est bien en mode Client avant d'envoyer quoi que ce soit
    if (settings.networkMode != NetworkMode.client) return '';
  ...
  /// ⚡ P1 FIX (C4): Helper HTTP GET avec timeout standard
  Future<http.Response> _safeGet(String url) {
    return http.get(Uri.parse(url), headers: _getHeaders()).timeout(_httpTimeout);
  }

  /// ⚡ P1 FIX (C4): Helper HTTP POST avec timeout standard
  Future<http.Response> _safePost(String url, {required String body}) {
    return http.post(Uri.parse(url), headers: _getHeaders(), body: body).timeout(_httpTimeout);
  }
  ...
  void _scheduleReconnect() {
    ...
    // ⚡ P1 FIX (M3): Backoff exponentiel pour les reconnexions
    _reconnectAttempts++;
    final delay = (_reconnectAttempts * 5).clamp(5, _maxReconnectDelay);
    debugPrint('🔄 Reconnexion WebSocket dans ${delay}s (tentative #$_reconnectAttempts)');
  ...
  Future<void> syncSettingsFromServer() async {
    ...
    try {
      final response = await _safeGet('$baseUrl/sync/settings');
  ...
  Future<void> syncUsersFromServer() async {
    ...
    try {
      final response = await _safeGet('$baseUrl/sync/users');
      if (response.statusCode == 200) {
  ...
  Future<void> syncProductsFromServer() async {
    ...
    try {
      ...
      final response = await _safeGet('$baseUrl/sync/products?since=$lastSync');
      if (response.statusCode == 200) {
  ...
  Future<bool> sendFinancialTransactionToServer(Map<String, dynamic> txMap) async {
    ...
    try {
      final response = await _safePost(
        '$baseUrl/sync/financial-transaction',
  ...
  /// Envoie un signal de suppression à distance au serveur (C7)
  Future<bool> sendDeleteToServer(String table, String id) async {
    final baseUrl = _getBaseUrl();
    if (baseUrl.isEmpty) return false;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        debugPrint('✅ Suppression à distance envoyée : $table -> $id');
        return true;
      }
      return false;
    } catch (e) {
      debugPrint('❌ Échec envoi suppression à distance : $e');
      return false;
    }
  }

  /// Envoie un produit modifié au serveur (Mode Client uniquement)
  Future<bool> sendProductToServer(Product product) async {
    ...
    try {
      ...
      payload['client_timestamp'] = DateTime.now().millisecondsSinceEpoch; // Pour la logique LWW du serveur
      
      final response = await _safePost(
        '$baseUrl/sync/product',
        body: jsonEncode(payload),
      );
      if (response.statusCode == 200) {
        debugPrint('✅ Produit ${product.name} envoyé au serveur avec succès.');
        // Marquer comme synchronisé localement
        final db = await _ref.read(databaseServiceProvider).database;
        await db.update('products', {'is_synced': 1}, where: 'id = ?', whereArgs: [product.id]);
        return true;
      } else {
        debugPrint('❌ Échec envoi produit (status ${response.statusCode})');
        return false;
      }
    } catch (e) {
      debugPrint('❌ Échec envoi produit: $e');
      final db = await _ref.read(databaseServiceProvider).database;
      await db.update('products', {'is_synced': 0}, where: 'id = ?', whereArgs: [product.id]);
      return false;
    }
  }
  ...
  /// ⚡ P1 FIX (M4): Debounce pour éviter les appels cascadés
  Future<void> syncPendingAuditData() async {
    final baseUrl = _getBaseUrl();
    if (baseUrl.isEmpty) return;
    
    // Debounce: max 1 appel par 30 secondes
    if (_lastAuditSync != null && DateTime.now().difference(_lastAuditSync!) < _auditSyncDebounce) {
      debugPrint('🕒 syncPendingAuditData debounced (max 1x/30s)');
      return;
    }
    _lastAuditSync = DateTime.now();
