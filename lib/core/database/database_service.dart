...
class DatabaseService {
  ...
  Future<void> runSafetyChecks(Database db) async {
    try {
      ...
      // 4. Session Integrity & Refund Tracking in sales
      ...
      final hasRefundedAtCol = salesInfo.any((col) => col['name'] == 'refunded_at');
      if (!hasRefundedAtCol) {
        debugPrint('Nuclear Fix: Adding missing refunded_at column to sales');
        await db.execute('ALTER TABLE sales ADD COLUMN refunded_at TEXT');
      }

      final hasRefundedByCol = salesInfo.any((col) => col['name'] == 'refunded_by_user_id');
      if (!hasRefundedByCol) {
        debugPrint('Nuclear Fix: Adding missing refunded_by_user_id column to sales');
        await db.execute('ALTER TABLE sales ADD COLUMN refunded_by_user_id TEXT');
      }
