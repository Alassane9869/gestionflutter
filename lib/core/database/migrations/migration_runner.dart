...
class MigrationRunner {
  ...
  static Future<void> _migrateToV51(Database db) async {
    final tables = [
      'sales',
      'products',
      'clients',
    ...
    for (final table in tables) {
      try {
        await db.execute('ALTER TABLE $table ADD COLUMN is_synced_to_cloud INTEGER NOT NULL DEFAULT 0');
      } catch (e) {
        debugPrint('Migration V51 warning for table $table: $e');
      }
      try {
        await db.execute('''
          CREATE TRIGGER IF NOT EXISTS trigger_${table}_cloud_sync
          AFTER UPDATE ON $table
          FOR EACH ROW
          WHEN (old.is_synced_to_cloud = 1 AND new.is_synced_to_cloud = 1)
          BEGIN
            UPDATE $table SET is_synced_to_cloud = 0 WHERE id = old.id;
          END;
        ''');
      } catch (e) {
